...
test("should format delivery instructions for Salesforce CSV parser", () => {
  const rawSpecialDeliveryInstructionsInSoldToZuoraContact =
    'front door is on "Foo\'s Drive ",Put though the letterbox, do not leave on door step.';
  const expected =
    "front door is on 'Foo's Drive ',Put though the letterbox, do not leave on door step.";
  expect(
    formatDeliveryInstructions(
      rawSpecialDeliveryInstructionsInSoldToZuoraContact
    )
  ).toEqual(expected);
});
...
test("guardian weekly output format for Salesforce CSV parser", async () => {
  ...
  const row = la.zipObject(gwOutputHeaders, [
    "A-S00000000",
    "Foo Bar",
    "",
    "Level 43 234 Foo St",
...
test("home delivery output format for Salesforce CSV parser", async () => {
  ...
  const row = la.zipObject(hdOutputHeaders, [
    "A-S00000000",
    "",
    "Foo Bar",
    "",
    ...
    "",
    "",
    'front door is on "Foo Bar",Put though the letterbox, do not leave on door step.',
    "",
    "",
    "19/01/2021",
    "20/01/2021",
