...
const normaliseTuple = (tuple: [string, string]): [string, string] => [
  normalise(tuple[0]),
  tuple[1],
];

const canadianStatesNotFormatted: [string, string][] = [
  ["Alberta", "AB"],
  ["British Columbia", "BC"],
  ["Manitoba", "MB"],
  ["New Brunswick", "NB"],
...
export const canadianStates = new Map<string, string>(
  canadianStatesNotFormatted.map(normaliseTuple)
);
...
const USStatesNotFormatted: [string, string][] = [
...
export const USStates = new Map<string, string>(
  USStatesNotFormatted.map(normaliseTuple)
);
