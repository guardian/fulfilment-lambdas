...
async function asyncHandler (input: input): Promise<Array<result>> { // eslint-disable-line no-use-before-define
  ...
  const files = batches.map(batch => zuora.fetchFile(batch, input.deliveryDate))
