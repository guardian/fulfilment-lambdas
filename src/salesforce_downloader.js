...
async function downloader () {
  ...
  const folders: Array<Folder & S3Folder> = [config.fulfilments.homedelivery.downloadFolder, ...Object.keys(config.fulfilments.weekly).map(k => config.fulfilments.weekly[k].downloadFolder)]
  const promises = folders.map(folder => download(config, salesforce, folder))
...
async function download (config: Config, salesforce: Salesforce, folder: Folder & S3Folder) {
  ...
  const keys = contents.map(r => { return r.Key.slice(folder.prefix.length) })
