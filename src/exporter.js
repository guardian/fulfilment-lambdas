...
exports.handler = (input, context, callback) => {
    ...
    function getDownloadStreamFromBucket(queryName) {
        ...
        if (!fileName) {
            callback(`Invalid input cannot find unique query called ${queryName}`);
        }
    ...
    let getHolidaySuspensions = new Promise((resolve, reject) => {
        ...
        let csvStream = csv.parse({
            ...
            .on('error', function (err) { 
                reject(`error reading holidaySuspensions: ${err}`)
            })
            .pipe(csvStream);      
    ...
    function processSubs(holidaySuspensions) {
        ...
        .on('error', function (err) { 
            callback(`error reading holidaySuspensions: ${err}`);
        })
        .pipe(csvStream);   
  ...
  if(stage != 'CODE' && stage != 'PROD') {
        callback(`invalid stage: ${stage}, please fix Stage env variable`);
        return;
    }
